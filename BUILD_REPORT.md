...
## Ω. PHASE Ω addendum (2026-08-31 — v0.1.7-rc2 luxury edition)

What was built in the product-refinement pass, all measured on this tree:

| Surface | What it is |
|---|---|
| `brand/` + `public/` + `tools/brand-render.ts` | The complete brand system: the Seal (witness rule above the V), monogram, custom-monoline wordmark, primary lockup, four editions (gold/white/black/mono), favicon + web icons, OG image, terminal mark. ALL vector + raster assets are generated by one deterministic script — run twice, byte-identical (verified via md5). `brand/BRAND-BOOK.md` + `brand/BRAND-BOOK.pdf` document the system. |
| `packages/vaerion/src/cli/ui.ts` | The terminal design language: render profiles (`json`/`plain`/`rich`, TTY-gated with `VAE_UI` override), ANSI painter (truecolor, `NO_COLOR`-safe), panels, tables (visible-width discipline + per-cell paint), badges, receipts, educated error blocks (code · catalog name · message · summary · Fix · docs anchor · related command), brand banner, quiet spinner (sanctioned SystemClock only, unref'd). |
| `src/cli/render.ts` + `vae.ts` + `commands.ts` | The Renderer v2: plain mode byte-compatible (every legacy assertion passes untouched); rich dispatch keyed on the stable `command` field with dedicated reports for doctor/dev/init/explain/serve/resume/run (model+agent+research)/journal/package/provenance; rich help frame; spinners on doctor + package build + run model. |
| `vae provenance` | The additive artifact-evidence command: `.vxn` (full pure format check — digests recomputed), `vaerion.lock` (seal cross-checked against the on-disk bundle, E2205 findings), `*.ndjson` export (derivation header), release MANIFEST (displayed as recorded). Exit 0 evidence holds · exit 5 findings. |
| Docs | README (seal, provenance step, design-language note, brand row), `docs/FAQ.md`, QUICKSTART provenance step, spec changelog 0.1.7-rc2. |

Measured: 290 tests / 1969 expectations / 0 failures; coverage floors held
(86.00% lines / 90.84% branches, text reporter; lcov cross-check 88.90%);
all six gates GREEN on this tree; version lockstep 0.1.7-rc2 across the
manifest surfaces; golden receipt re-blessed for the lockstep bump.
