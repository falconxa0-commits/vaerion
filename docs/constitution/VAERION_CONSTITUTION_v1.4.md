...
| 7 | ASCENSION XVIII | ▶ in flight | re-issued under A4 with a recorded definition — the performance budget law (one deterministic harness, typed budgets, one verify.ts gate step); executes the §7 MS-6 performance double-check |
| 8 | ASCENSION XVIII | ▶ in flight | ratified under A4 — the accessibility law (deterministic structural checker + browser-measured audit + behavior-pinned color accessibility + docs sweep); executes the §7 MS-6 accessibility sweep |
| 9 | ASCENSION XVIII | ▶ in flight | ratified under A4 — the release-train rehearsal (one deterministic end-to-end runner + measured rehearsal report); executes the §7 GA "release train rehearsed" |
