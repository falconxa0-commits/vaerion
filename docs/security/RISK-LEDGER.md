...
| R-2 | high | Supply chain | Release signature verification currently trusts the bootstrap Ed25519 public key shipped in the repository; no offline key ceremony has taken place | Founder | Key ceremony performed; release public key rotated to a held-offline key; revocation procedure published | **CLOSED (ASCENSION XXV, Phase XXVII)** — production key provisioned as `RELEASE_SIGNING_KEY`; key of record rotated; ceremony + rotation + recovery law in `SIGNING-CEREMONY.md`. Residual (labeled): the key was generated in a session under the Founder's written directive, not air-gapped — a hardware-custodied re-ceremony remains available via the recorded rotation path |
