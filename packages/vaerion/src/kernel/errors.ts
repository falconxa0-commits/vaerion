...
export const ERROR_CATALOG: Readonly<Record<ErrorCode, ErrorDescriptor>> = {
  ...
  E2305: { code: "E2305", name: "ci_verify_authority_missing", summary: "A workflow job runs gate logic without invoking tools/verify.ts — the single verification authority (D-R).", fix: "Make every verification job run `bun run tools/verify.ts`; no surface may re-implement the seven gates." },
