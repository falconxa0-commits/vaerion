...
const milestones = [
  ...
  { id: "MS-5", name: "Surfaces", status: "in_progress", progress: 75, evidence: "DONE this sprint: local API daemon (ADR-0010/ADR-0020) — loopback Bun.serve listener with pairing-token authn (CSPRNG, print-once, VAE_TRUST pre-provision, timing-safe compare); route table as the single source for dispatch AND the generated spec/openapi.json (C4 byte-sync); agent/workflow runs over the wire with the SAME engine composition as the CLI (serial run queue protecting the single-writer audit/refusal chains); SSE event streams with journal-cursor replay + follow-to-receipt and redaction-before-publication; workspace-wide merged event tail; durable gate answer/continue over the wire (elevation law); receipted cancellation for paused/interrupted runs; models/tools capability surfaces (secret NAMES only); POST /shutdown token-echo guard; `vae serve` CLI command (additive eighth command, Daily Seven unchanged); SDK wire client (daemon-transport.ts — the ONE sanctioned loopback-enforced client site, E2006) + VaeDaemonClient with parity tests proving identical journaled event-type sequences vs the in-process client; new checks: layerlint api/→L4 sibling-surface rule, C7 listener-egress-freedom, C4 openapi sync; spec 0.1.4 (openapi.json + E2000–E2006); coverage ratcheted to measured 85.23/90.16. REMAINING: extension kit alpha (WIT/host per ADR-0009, contingency R-2), optional blueprint route groups (sessions/intel/packages) deferred until their subsystems ship." },
...
const status = {
  ...
  verification,
  tests: { suites: 22, assertedExpectations: 1700, totalTests: 237, coverage: { lines: 85.23, branches: 90.16, floors: "bunfig.toml coverageThreshold (OBJ-Q6, ratcheted at MS-5 daemon close: 0.85/0.74/0.85/0.89)" }, note: "counts from the latest full run of `bun test tests/ --coverage`" },
  ...
  risks: [
    ...
    "Extension kit alpha (ADR-0009 WIT/host) is the remaining MS-5 exit criterion; Bun's WASI support must be evaluated against the fail-closed capability law before the host lands.",
  ],
  nextWork: [
    "MS-5 completion: extension kit alpha (WIT locked, host fail-closed per ADR-0009/contingency R-2); optional blueprint route groups (sessions/intel/packages) once their subsystems warrant wire exposure.",
