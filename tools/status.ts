...
const status = {
  ...
  verification,
  tests: { suites: 25, assertedExpectations: 1858, totalTests: 278, coverage: { lines: 86.07, branches: 90.87, floors: "bunfig.toml coverageThreshold (OBJ-Q6, ratcheted at MS-6 bundle close: 0.86/0.74/0.86/0.90)" }, note: "counts from the latest full run of `bun test tests/ --coverage`" },
  ...
  nextWork: [
    "MS-6 remaining exit criteria: installers, docs sweep, accessibility, performance double-check; then the daemon packages route group (wire parity, spec/openapi regen) now that the packaging subsystem exists.",
